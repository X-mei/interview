...
* 能访问private & protected，定义在类外，不是成员函数
* 破坏封装性
* 友元关系不可传递，不能被继承
* 友元关系的单向性，友元可以访问有friend关键字一边，反过来不可以
* 友元声明的形式及数量不受限制
...
#### 特点
1. 枚举常量表不可以重复，只能以标识符形式如 y200，不能是整型等文字常量如 200.
...
* 互斥：一个资源每次只能被一个进程使用，即在一段时间内某资源仅为一个进程所使用。此时如果有其他进程请求该资源，则请求进程只能等待。
* 请求和保持：进程中已经保持了至少一个资源，但又提出了新的资源请求，而该资源已经被其他进程占有，此时请求进程被阻塞，但对自己已经获得资源保持不放。
* 不剥夺：进程未使用完的资源在未使用完毕之前，不能被其他进程强行夺走，即只能由获得该资源的进程自己来释放。
* 环路：若干进程间形成首尾相接循环等待资源的关系。在发生死锁时必然存在一个进程等待队列{P1，P2，…,Pn},其中P1等待P2占有的资源，P2等待P3占有的资源，…，Pn等待P1占有的资源，形成一个进程等待环路，环路中每一个进程所占有的资源同时被另一个申请。
